...
**27:** Strain rate and polar decomposition: $\mathbf{D}_p = 0.5(\mathbf{L}_p^{t+\Delta t} + {\mathbf{L}_p^{t+\Delta t}}^{\mathrm{T}})$, $\quad\mathbf{F}_p^{t+\Delta t} \xrightarrow{\text{polar}} \mathbf{R}_p,\ \mathbf{U}_p$
**28:** Un-rotated strain rate and deviatoric part: $\mathbf{d}_p = \mathbf{R}_p^{\mathrm{T}}\mathbf{D}_p\mathbf{R}_p$, $\quad\mathbf{d}_p^d = \mathbf{d}_p - \tfrac{1}{3}\mathrm{tr}(\mathbf{d}_p)\,\mathbf{I}$
...
| $\dot{\varepsilon}_p^*$ | Normalized plastic strain rate $= \dot{\varepsilon}_p / \dot{\varepsilon}_0$ |
